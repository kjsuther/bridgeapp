/*
  ...
  1. New Tables
  - `hand_cards` — one row per seat per hand, storing that seat's cards as jsonb.
    Primary key is (hand_id, seat). Foreign key to hands(id) with cascade delete.
    RLS enabled with a SELECT policy that reveals:
      a) your own seat's cards,
      b) dummy's cards after the opening lead (dummyRevealed flag),
      c) all seats once the hand reaches scoring or finished phase.
  2. Modified Tables
  - `hands` — added `state_version bigint NOT NULL DEFAULT 0` for optimistic
    concurrency control. The legacy `deal` column is relaxed to nullable and
    cleared of existing data (migrated into hand_cards first).
  - `table_players` — SELECT policy tightened so a player can only see other
    seats at tables they are a member of (or their own row).
  - `plays` — SELECT policy tightened to require table membership. Old insert
    policies dropped (writes now go through SECURITY DEFINER functions).
  3. New Functions (all SECURITY DEFINER, execute granted to authenticated only)
  - `can_access_bridge_table(uuid)` — helper: true if auth.uid() is the host or
    a seated player at the given table.
  - `create_bridge_hand(game_id, hand_number, dealer, vulnerability)` — shuffles
    and deals inside Postgres, inserting into hand_cards. Returns the new hand id.
  - `update_bridge_hand(hand_id, expected_version, patch)` — optimistic-lock
    update of auction/contract/score/play_state/bid_undo_request/phase.
  - `record_bridge_play(hand_id, trick_number, seat, card)` — validates and
    inserts a play. Returns the new play id.
  - `delete_last_bridge_play(hand_id)` — removes the most recent play.
  4. Security Changes
  - RLS enabled on hand_cards with visibility-based SELECT policy.
  - table_players SELECT policy restricted to self or table members.
  - plays SELECT policy restricted to table members.
  - Old permissive policies dropped: hands_update_table_member,
    plays_insert_member, plays_insert_table_member, table_players_select_all,
    table_players_select_visible (replaced).
  5. Indexes
  - Unique index on plays(hand_id, trick_number, seat) to enforce one card
    per seat per trick.
  6. Realtime
  - hand_cards added to the supabase_realtime publication (if it exists).
*/
